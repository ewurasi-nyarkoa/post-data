...
export class DetailPostComponent implements OnInit {
  ...
  private loadPost(): void {
    ...
    // First check if post exists in state
    ...
    this.apiClient.get<Post>(
      ...
      { key: `post-${id}`, duration: 3600000 }, // Cache for 1 hour
  ...
  onDelete(): void {
    ...
    if (confirmed) {
      this.apiClient.delete(
        ...
        [this.POSTS_STATE_KEY], // Invalidate posts cache
        { stateKey: this.POSTS_STATE_KEY, itemId: this.post.id } // Remove from state
      ).subscribe({
        next: () => {
          alert('Post deleted successfully');
          this.router.navigate(['/posts']);
  ...
  fetchComments(): void {
    ...
    this.apiClient.get<Comment[]>(
      ...
      { key: `comments-${this.post.id}`, duration: 1800000 }, // Cache for 30 minutes
